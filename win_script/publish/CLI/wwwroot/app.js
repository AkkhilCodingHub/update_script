...
function renderSteps() {
    ...
    const filtered = stepsData.filter(s => s.isAvailable && (activeCategory === 'all' || s.category === activeCategory));

    if (filtered.length === 0) {
        stepsList.innerHTML = '<div class="log-entry system-msg" style="padding: 20px; text-align: center;">No installed modules detected for this category.</div>';
        return;
    }
...
function updateStats() {
    ...
    statAvailable.textContent = `${available} Installed`;
}
